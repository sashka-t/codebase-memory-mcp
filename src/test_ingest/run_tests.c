...
static int validate_tokens(const char *cmd, char **out_error, char **out_stdout, size_t *out_len,
                           int *out_exit_code) {
    const char *p = cmd;
    while (*p) {
        while (*p == ' ' || *p == '\t') {
...
int cbm_run_tests_exec(const char *command, const char *cwd, char **out_stdout, size_t *out_len,
    ...
    if (validate_tokens(command, out_error, out_stdout, out_len, out_exit_code) != 0) {
        return -1;
    }
